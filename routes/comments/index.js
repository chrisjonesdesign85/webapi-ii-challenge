        ...
        await Posts.insertComment({ text, post_id })
            .then(comment => {
                if (comment) {
                    ...
                    res.status(404).json({ success: false, message: 'The post with the specified ID does not exists.' });
                }
            })
            .catch((error) => {
                res.status(500).json({
                    success: false, errorMessage: `There was an error while saving the comment to the database. 
                    post id: ${req.params.id} comment text: ${req.body.text} error: ${error}`
                });
            });
    }
});
